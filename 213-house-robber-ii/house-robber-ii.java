class Solution {
    ...
    public int rob(int[] nums) {
        ...
        int robLast = dp(nums, 1, nums.length - 1, dpTableOne);
        int doNotRobLast = dp(nums, 0, nums.length - 2, dpTableTwo);
        return Math.max(robLast, doNotRobLast);
    }
    private int dp(int[] nums, int start, int end, Integer[] dpTable) {
        ...
        dpTable[start] = Math.max(doNotRob, rob);
        return dpTable[start];
    }
}